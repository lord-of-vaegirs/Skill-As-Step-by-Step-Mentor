...
A Codex skill-agent project for source-grounded, hands-on engineering learning. It turns a broad learning goal into an adaptive route, verifies practical evidence one part at a time, and persists recoverable Markdown progress.

After the learner confirms a topic, reference materials, work directory, and final goal, Codex derives an ordered route and teaches exactly one part at a time. Every part is strictly gated: explanation only, `[finish]`, practice and review, then `[summarize]`. Later-part content remains hidden until preceding parts are summarized, and `[summarize all]` unlocks the final synthesis only after the last part. New technical terms are defined before first use, explanations build from source-grounded background step by step, and recoverable progress records the active gate. Restart keeps Q&A while clearing reset theory, practice, evidence, and completion state so the affected parts are learned again.
