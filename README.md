...
By default, every learning part explains the relevant knowledge before presenting answers or practice. New technical terms and abbreviations are defined before first use and retained in the progress file. Explanations move step by step from background and prerequisites through structures, operations, examples, and bounded practice, using learner-provided materials to guide the route. If no final goal is supplied, Codex proposes an observable goal with success evidence and asks the learner to approve or revise it before the route begins.
