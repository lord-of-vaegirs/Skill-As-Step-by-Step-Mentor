...
学习者确认学习主题、参考资料、工作目录和最终目标后，Codex 会推导有序学习路线，并严格一次只推进一个部分。每部分固定遵循“仅讲解 → `[finish]` → 练习与审查 → `[summarize]`”；前一部分尚未总结时，不会出现后一部分的学习内容。最后一部分总结后，只有 `[summarize all]` 才会生成最终总总结。首次出现的专业名词会先得到解释，讲解会依据资料从背景和前置知识逐步展开，进度文件会记录当前阶段。`[restart]` 保留问答记录，但清除回退范围内的理论学习、练习、证据和完成状态，以便重新学习。
