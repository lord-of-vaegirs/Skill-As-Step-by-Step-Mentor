...
默认情况下，每次学习和每个学习部分都会先讲解相关知识点，再给出答案或练习；只有学习者明确表示进展非常顺利或已经掌握相关内容时，才会跳过或压缩讲解。首次出现的专业名词、简称和术语会先得到解释，并记录到进度文件中。讲解会参考学习者提供的资料，按照背景、前置知识、术语、结构、操作、示例和练习逐步展开。如果学习者没有提供最终目标，Codex 会自主提出包含成功证据的可观察暂定目标，并在学习路线开始前交由学习者确认或修改。
